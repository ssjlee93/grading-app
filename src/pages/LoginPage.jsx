...
export default function LoginPage() {
  ...
  return (
    <div className="flex min-h-screen">
      {/* Left — branding panel */}
      <div className="hidden w-1/2 flex-col justify-between bg-ink-950 p-12 text-ink-100 lg:flex">
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-white/10">
            <GraduationCap className="h-6 w-6 text-white" />
          </div>
          <span className="font-display text-2xl text-white">GradeBook</span>
        </div>

        <div className="max-w-md">
          <h2 className="font-display text-5xl leading-tight text-white">
            Where grades <br />
            <span className="italic text-accent-light">find clarity.</span>
          </h2>
          <p className="mt-4 text-lg leading-relaxed text-ink-400">
            A streamlined portal for teachers, students, and administrators to manage
            academic performance — no spreadsheets required.
          </p>
        </div>

        <p className="text-sm text-ink-600">
          &copy; {new Date().getFullYear()} GradeBook. Built with Firebase.
        </p>
      </div>
      ...
      <div className="flex flex-1 items-center justify-center p-8">
        <div className="w-full max-w-md animate-fade-in">
          ...
          <div className="mb-8 flex items-center gap-3 lg:hidden">
            ...
            <span className="font-display text-xl text-ink-950">GradeBook</span>
          </div>
          ...
          <p className="mt-2 text-ink-500">
            {isRegister
              ? 'Sign up to get started with GradeBook.'
              : 'Sign in to access your grading portal.'}
          </p>
          ...
          <form onSubmit={handleSubmit} className="mt-8 space-y-4">
            ...
            <button type="submit" disabled={loading} className="btn-primary w-full">
              {loading ? (
                <div className="h-4 w-4 animate-spin rounded-full border-2 border-ink-400 border-t-white" />
              ) : (
                <>
